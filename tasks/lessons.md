...
## 2026-03-23
- 用户纠偏：架构教程不是把“问题是什么、为什么、如果不这样会怎样”机械铺满就够了；过多解释性转场和重复框架会把文风写得啰嗦，失去顶级教程应有的密度、节奏和舒服感。
- 防错规则：写教程型文档时，先压缩结构层级和重复句式，优先写高密度结论、自然推进的段落和少量真正有信息量的小节；不要把每章都写成模板化问答，也不要把显而易见的转场全部说出来。
- 用户继续纠偏：当前产物连“教程”都不像，更像质量很差的草稿；说明问题不只是篇幅控制失败，而是没有先基于成熟教程方法论校准“教程是什么、目标是什么、好教程怎么组织”。
- 防错规则：当任务目标是写“教程”而不是普通说明文时，先调研成熟教程框架和权威写作准则，再定义教程的读者、学习目标、章节推进、叙述节奏和示例策略；未完成方法论校准前，不要直接开写正文。
- 用户继续纠偏：像 `s00` 这种“先讲世界观”的预备章，如果没有明显必要，就属于多余层；教程不该为了结构完整感强行增加起手章节。
- 防错规则：设计教程章节时，先验证每一章是否承担不可替代的学习任务；如果某章只是铺垫、没有独立学习收益，就合并到 README 或后续正文，不单列成章。
- 用户继续纠偏：不要把“教程正文”和“面试表达”当成两套写法来回切换；正文只要把机制和因果链写清楚，教程和面试复述自然都会成立。
- 防错规则：写教程正文时，不要额外叠加“面试层”“总结层”“表达模板层”；优先把核心对象、问题、机制、约束和后果写清楚，让复述能力从清晰正文自然长出来。
- 用户继续纠偏：即使删掉模板腔，如果正文仍然只是“概念解释 + 自我总结”，也还是很差；真正的问题是文风、视角、推演方式都没达到用户认可的教程标准。
- 防错规则：当用户连续两次否定教程质量时，停止凭主观感觉继续重写；必须先拿到一篇用户认可的参考样式，抽取其密度、节奏、句法和推进方式，再按那个标尺重写。
