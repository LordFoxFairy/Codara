...
export const teamCommand: CodaraCommandDefinition = {
  name: 'team',
  usage: '/team <create|list|status|health|jobs|pause|resume|kill|finish|enter|leave|message|logs|assign>',
  ...
  async execute({command, agent}) {
    ...
    switch (sub) {
      ...
      case 'logs': {
        ...
        const team = registry.getTeamByName(name) ?? registry.getTeam(name);
        if (!team) return err(command.name, `Team "${name}" not found.`);
        const allMessages = runtime.getTeamMessages(team.teamId);
        if (allMessages.length === 0) return ok(command.name, 'No messages available for this team.');
      ...
      default:
        return ok(command.name, [
          ...
          '  /team health <name>              Show team health (deadlock, members)',
