...
function buildAgentChildMiddlewares(options: CreateAgentMiddlewareOptions): BaseMiddleware[] {
  ...
  if (options.childRuntime?.hil !== false && !seen.has(MIDDLEWARE_NAMES.AskUserQuestion)) {
    push(createAskUserQuestionMiddleware());
  }
  if (options.childRuntime?.hil !== false && !seen.has(MIDDLEWARE_NAMES.Permission)) {
    push(createPermissionMiddleware({
      ...(typeof options.childRuntime?.hil === 'object' && options.childRuntime.hil !== null
        ? options.childRuntime.hil
        : {}),
