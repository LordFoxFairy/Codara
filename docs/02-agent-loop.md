...
- AgentLoop 的主判断信号（`tool_calls`）和辅助信号（`stop_reason`）如何协作。
- 中间件钩子如何把安全、权限、审计从循环本体解耦。
- 事件回调模式如何把循环与 TUI/CLI 消费层解耦。

### 建议阅读方式

1. 先看「核心循环」，抓住每轮执行骨架。
2. 再看「工具执行流程」，理解 PreToolUse 到权限再到执行的顺序。
3. 最后看「事件流 + 安全阀 + 生命周期」，建立稳定性心智模型。

### 完成标志

- 你能定位一个行为应该放在哪个中间件钩子，而不是改循环主干。
- 你能解释权限弹窗为何通过 `permission_request` 事件回调实现。

### 最小实操

1. 跑一次包含工具调用的任务，记录事件顺序：`turn_start -> text_delta -> tool_start -> tool_end -> done`。
2. 针对同一工具调用，核对中间件关键顺序是否符合本文工具执行链。
3. 触发一次 `ask` 权限路径，观察 `permission_request` 发出与 `resolve` 恢复执行的过程。

### 常见误区

- 在 AgentLoop 主循环中堆业务分支，而不是下沉到中间件。
- 把“无 tool_calls”误判为错误路径，实际上这通常是正常完成信号。

### 排错清单（症状 -> 排查顺序）

| 症状 | 排查顺序 |
|------|----------|
| 循环提前结束 | 检查 `tool_calls` 是否为空 -> 检查 `done` 原因 -> 检查安全阀 |
| 权限弹窗不出现或卡住 | 检查 PermissionMiddleware -> 检查 `permission_request` 事件与 `resolve` 回调 |
| 工具执行顺序异常 | 对照“工具执行流程”检查各中间件挂载顺序和 pre/post 阶段 |
...
详见 [05-记忆与上下文](./05-memory-system.md)。
...
完整中间件架构、接口定义和内置中间件清单详见 [05-中间件与钩子](./04-hooks.md)。
