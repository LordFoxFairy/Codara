...
**预留通道（Multi-Channel 准备）：**
- Telegram / DingTalk / Feishu / QQ / WeCom
...
## 附录 A：从 v0.7 到 v1.0 的重构映射

| v0.7（旧目录） | v1.0（目标目录） | 变更类型 |
|----------------|-----------------|---------|
| `engine/agent/` | `core/agent/` | 重命名 |
| `engine/pipeline/` | `core/pipeline/` + `core/middleware/` | 拆分 |
| `engine/session/` | `durability/session/` | 迁移 |
| `engine/checkpoint/` | `durability/checkpoint/` | 迁移 |
| `engine/events/` | `observability/events/` | 迁移 |
| `engine/hook/` | `observability/hook/` | 迁移 |
| `engine/mcp/` | `integration/mcp/` | 迁移 |
| `engine/tool/` | `integration/tool/` | 迁移 |
| `engine/channel/` | `integration/channel/` | 迁移 |
| `infra/provider/` | `integration/provider/` | 迁移 |
| `infra/context/` | `context/` | 提升为一级目录 |
| `infra/config/` | `config/` | 提升为一级目录 |
| `capability/` | `capability/` | 保持 |
| `codara/` | `codara/` | 保持 |
| `cli/` | `cli/` | 保持 |
| `desktop/` | `desktop/` | 保持 |
| `server/` | `server/` | 保持 |
| `bus/` | `bus/` | 保持 |
| `shared/` | `shared/` | 扩展 contracts |

### 重构影响评估

- **拆解 `engine/`**：~40 个文件迁移，影响最大
- **拆解 `infra/`**：~20 个文件迁移
- **路径别名更新**：`@engine/*` → `@core/*`, `@durability/*`, `@observability/*`, `@integration/*`
- **barrel export 重写**：所有 index.ts 重新组织
- **测试更新**：import 路径批量替换
- **公共 API 不变**：`src/index.ts` 导出保持稳定，对外无破坏性变更
