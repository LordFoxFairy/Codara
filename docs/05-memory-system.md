...
- MEMORY、Session、Checkpoint 三类记忆机制的职责边界。
- 为什么“记忆系统”不等于“上下文压缩/Token 统计”。
- 如何在不污染长期记忆的前提下维护稳定项目知识。

### 建议阅读方式

1. 先看「在主线中的定位」明确边界。
2. 再按 `MEMORY.md → Session → Checkpoint` 顺序理解数据生命周期。
3. 最后看「架构总结」把三者与 AgentLoop 的连接关系串起来。

### 完成标志

- 你能判断一条信息应写进 `MEMORY.md`、会话历史，还是不应持久化。
- 你能解释一次文件回退为什么能恢复到修改前状态。

### 最小实操

1. 手动添加一条稳定偏好到 `MEMORY.md`，重启会话确认被重新注入。
2. 在一次任务中执行两次文件修改，再触发回退验证 checkpoint 恢复行为。
3. 查看 session 文件，确认会话历史与元数据被正确持久化。

### 常见误区

- 把一次性任务细节写进长期记忆，导致记忆污染。
- 把压缩和 token 统计误归类为记忆系统职责。

### 排错清单（症状 -> 排查顺序）

| 症状 | 排查顺序 |
|------|----------|
| 重启后“记不住”项目偏好 | 检查 `MEMORY.md` 路径 -> 检查是否写入前 200 行 -> 检查会话是否重建系统提示 |
| 回退后文件未恢复 | 检查是否创建 checkpoint -> 检查回退目标 checkpoint ID -> 检查回退模式 |
| 会话恢复内容不完整 | 检查 session 文件完整性 -> 检查原子写入流程 -> 检查恢复时系统消息重建逻辑 |
