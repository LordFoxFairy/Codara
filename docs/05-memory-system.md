...
1. MEMORY 只记录稳定事实和长期偏好，不记录短期执行噪声。
2. 写入类工具必须先快照后修改，保证检查点可回退。
3. 会话持久化必须原子化，避免部分写入导致损坏。
4. 压缩只影响模型输入上下文，不直接改写检查点快照。
5. 任意恢复路径都必须保留可解释的时间点和变更来源。
...
以下字段在任何压缩阶段都必须完整保留，不可被摘要或裁剪：

| 类别 | 字段 | 原因 |
|---|---|---|
| 会话标识 | `session.id`、`createdAt`、`lastActive` | 保证会话可追踪与恢复 |
| 运行计量 | `totalTurns`、`totalCostUsd`、`model` | 保证计量一致性与审计能力 |
| 检查点索引 | `checkpoint.id`、`time`、`filesChanged` | 保证回退定位可用 |
| 最近工具事实 | 最近一轮工具调用结果摘要 | 保证下一轮推理不失真 |

### 压缩失败降级策略

当压缩过程失败或输出不可用时，按以下顺序降级：

1. 保留原上下文并重试一次轻量压缩（仅裁剪长工具输出）。
2. 若仍失败，停止自动压缩，发出可观测事件并提示人工触发。
3. 若上下文已无法继续执行，终止当前轮次并返回明确错误原因。
...
1. 目标检查点必须存在且可解析。
2. 回写文件数应与快照集合可对应。
3. 回退后检查点链应截断到目标点，不保留“悬空检查点”。
4. 会话消息与检查点时间线不冲突（至少保持顺序一致）。
...
- 上下文压缩与记忆协同：用于容量治理，但不改变检查点回退语义
- Token 追踪属于计量层，不属于记忆层职责
