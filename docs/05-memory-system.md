...
记忆系统中的压缩能力用于控制长会话上下文体积，避免超出模型窗口。其与会话/记忆的协同关系如下：

1. 触发条件：当会话上下文接近窗口上限（典型阈值 95%）时触发压缩。
2. 第一阶段：优先裁剪长工具输出，保留关键首尾信息。
3. 第二阶段：若仍超阈值，对较早消息进行摘要，保留近期可执行上下文。
4. 边界要求：压缩不应破坏检查点与会话元数据的一致性；仅影响模型输入上下文，不直接篡改检查点快照内容。

这意味着：记忆层既负责长期保存（MEMORY/session/checkpoints），也负责运行时“可继续推理”的上下文体积治理。
