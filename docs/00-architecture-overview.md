...
- Codara 的分层边界：核心运行时 vs Skills 扩展层。
- 为什么“机制进核心、策略进 skill”是长期可维护的设计。
- 从目录结构反推系统职责分工的方法。

### 建议阅读方式

1. 先看「核心设计理念」理解总原则。
2. 再看「目录结构 + 模块依赖关系图」建立代码地图。
3. 最后结合「启动流程」去 `architecture-runtime` 看完整时序。

### 完成标志

- 你能解释某个需求应该放在核心还是 skill。
- 你能从 `src/` 目录快速定位模型、循环、工具、hooks、skills 的实现入口。

### 最小实操

1. 选一个需求（例如“拦截危险命令”），先判断它属于机制还是策略。
2. 如果是策略，尝试写成 skill 方案（hooks + permissions）；如果是机制，再定位核心模块。
3. 对照本文“职责边界表”检查放置位置是否合理。

### 常见误区

- 把团队流程直接写进核心运行时，而不是技能层。
- 在设计讨论中先谈实现，忽略“机制/策略”边界判断。

### 排错清单（症状 -> 排查顺序）

| 症状 | 排查顺序 |
|------|----------|
| 新需求落地后结构越来越乱 | 回看“核心与 Skills 职责边界” -> 判断机制/策略 -> 再决定落点 |
| 维护者找不到代码入口 | 先看“目录结构” -> 再看“模块依赖关系图” -> 最后对照 `architecture-runtime` |
| 核心模块变更影响面失控 | 检查是否误把策略写进核心 -> 评估是否应改为 skill 编排 |
