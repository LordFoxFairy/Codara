...
1. 别名解析必须是单入口：所有模型选择统一走路由器。
2. 路由层只决定“用哪个模型”，不承载业务策略。
3. 路由结果必须可解释：至少包含 `provider`、`modelId`、密钥来源。
4. 解析失败必须 fail-fast，不允许静默回退到未知模型。
5. 子代理与主代理使用同一解析管道，不允许分叉语义。
...
3. 如果完全没有可用提供商，立即报错并终止初始化（不做隐式兜底）。
...
| 失败点 | 典型触发 | 建议处理 |
|---|---|---|
| 别名不存在 | `router` 中无映射，且无法推断提供商 | 明确报错，要求补充映射 |
| 提供商不存在 | `provider:model` 的 provider 未在 `providers` 定义 | 校正 provider 名称或补全配置 |
| 模型不在白名单 | `modelId` 不在 provider `models` 列表 | 修正别名目标或扩充 models |
| API 密钥不可用 | 环境变量缺失、空值或无效 | 明确提示缺失变量名 |
| 工具绑定不支持 | 模型实例无 `bindTools` 能力 | 更换支持工具调用的模型 |
