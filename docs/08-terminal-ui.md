...
> [← 上一篇: 代理协作](./07-agent-collaboration.md) | [目录](./README.md) | [下一篇: Team 运行时 →](./09-team-runtime.md)
...
| 项 | 契约定义 |
|---|---|
| 输入 | `AgentEvent` 事件流、用户输入、权限/问题交互回调 |
| 输出 | 终端渲染、用户决策回传（`resolve`）、运行状态可视化 |
| 主路径 | 事件消费 -> 组件渲染 -> 用户交互 -> 回调恢复循环 |
| 失败路径 | 终端尺寸受限、渲染溢出、事件乱序、交互超时 |

实现约束：
- UI 不改写运行时语义，只负责展示和交互回传。
- 双向事件（如 `permission_request`、`ask_user`）必须可恢复，不得造成死等。
- UI 层错误应降级渲染，不应导致会话主循环异常终止。

## UI 非目标

1. 不在 UI 层实现业务策略判断（策略在 hooks/permissions/skills）。
2. 不在 UI 层定义工具授权规则（只展示并回传用户决策）。
3. 不在 UI 层维护代理执行状态机真相源（真相源在 Agent Loop）。
...
```
╭─⏺ Bash: npm test           （运行中）
╭─✓ Read: project/main-entry       （完成，无输出）
╰─done
╭─✓ Bash: git diff            （完成，有输出）
╰─M  project/app-module
   A  project/utils-module
...
| `Read` | `Read: {file_path}` | `Read: project/auth-module` |
| `Write` | `Write: {file_path} ({N} lines)` | `Write: project/config-module (42 lines)` |
| `Edit` | `Edit: {file_path} ({old}→{new} lines)` | `Edit: project/auth-module (3→5 lines)` |
| `Glob` | `Glob: {pattern}` | `Glob: project/**/*` |
| `Grep` | `Grep: {pattern} in {path}` | `Grep: "validateToken" in project/` |
...
# 安全操作 (Read, Glob, Grep)
╭── theme.info (蓝色边框) ──────────────────────────────────────╮
│  ⚡ Read: project/auth-utils-module                                   │
│  Allow? [Y] Yes  [N] No  [A] Always  [S] Session             │
╰───────────────────────────────────────────────────────────────╯

# 修改操作 (Edit, Write)
╭── theme.warning (黄色边框) ───────────────────────────────────╮
│  ⚡ Edit: project/auth-utils-module                                   │
...
```
╭── theme.warning ──────────────────────────────────────────────╮
│  ⚡ Write: project/config-module (42 lines)                           │
│    + import { z } from "zod";                                 │
│  Allow? [Y] Yes  [N] No  [A] Always  [S] Session             │
╰───────────────────────────────────────────────────────────────╯
```
...
本章给出人在回路交互的运行契约，详细交互样式与组件行为见前文：

- `PermissionDialog`：见 3.6
- `QuestionDialog`：见 3.7
- `ConfirmDialog`：见 3.8
- 事件生命周期：见 4.3
...
| 事件 | 触发来源 | UI 动作 | 回调结果 |
|---|---|---|---|
| `permission_request` | 权限层命中 `ask` | 显示 PermissionDialog | `allow_once` / `deny` / `always_allow` / `allow_session` |
| `ask_user` | `AskUserQuestion` 工具 | 显示 QuestionDialog | 结构化答案对象 |
| `confirm_request` | 需要明确确认的流程点 | 显示 ConfirmDialog | `{ approve, feedback? }` |

### 11.2 同步阻塞语义

1. 事件发出后，Agent 循环进入等待态，直到 `resolve(result)` 被调用。
2. UI 必须保证每个待决事件恰好回调一次，避免重复提交或悬挂等待。
3. 若用户取消，需返回明确拒绝/取消结果，而不是丢弃事件。
4. 回调完成后，循环从同一执行点恢复，继续推进后续中间件链。

### 11.3 失败与超时处理

1. 对话框渲染失败时，回退为最小文本交互，不中断会话。
2. 用户无输入超时（如配置启用）时，采用保守决策（默认拒绝或返回空答案）。
3. 所有失败路径都应产生可观测事件和日志，避免静默吞错。

### 11.4 与权限/钩子的边界

1. UI 只负责展示与回传，不做权限规则求值。
2. `PreToolUse` 已拒绝的调用不应再次弹出权限对话框。
3. `deny > ask > allow` 的裁决顺序在权限层生效，UI 不改变该语义。
4. 会话结束时应清理所有待决对话状态，避免残留阻塞。
