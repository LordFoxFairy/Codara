...
- TUI 如何消费 AgentEvent 流并映射到消息、弹窗、状态栏。
- 输入区、弹窗区、消息区的布局关系和状态切换。
- 如何按“事件 -> 状态 -> 组件”顺序调试 UI 问题。

### 建议阅读方式

1. 先读「全局布局架构」和「组件层级」理解渲染骨架。
2. 再读 `PermissionDialog/QuestionDialog/ConfirmDialog` 把人在回路交互串起来。
3. 最后读事件分发章节，对齐 `AgentEvent` 到 UI 状态更新路径。

### 完成标志

- 你能定位一个 UI 异常是事件分发问题、状态管理问题还是组件渲染问题。
- 你能解释权限弹窗为何以内联方式出现在 InputArea 上方。

### 最小实操

1. 触发一次需要审批的工具调用，观察 `permission_request -> PermissionDialog -> resolve` 全链路。
2. 触发一次 `ask_user` 场景，验证 QuestionDialog 的多问题流程与返回结构。
3. 在长输出场景观察 `<Static>` 与流式区域切换，确认 scrollback 行为符合预期。

### 常见误区

- 只看组件样式不看事件分发，导致排错方向偏离。
- 忽略输入区禁用状态与弹窗状态联动，造成交互冲突误判。

### 排错清单（症状 -> 排查顺序）

| 症状 | 排查顺序 |
|------|----------|
| 权限弹窗不显示 | 检查是否收到 `permission_request` 事件 -> 检查 dialog 状态渲染分支 |
| 流式文本卡住或丢失 | 检查 `text_delta` 累积逻辑 -> 检查 flush 时机 -> 检查 turn 切换 |
| 输入框状态异常（可输/禁用错位） | 检查 `isRunning` 与 dialog 状态联动 -> 检查 InputArea 条件渲染 |
