...
4. 技能钩子（`.codara/skills/*/hooks/hooks.json` 和 `~/.codara/skills/*/hooks/hooks.json`）
...
## 技能与代理 Frontmatter 中的钩子

除了 `settings.json` 和技能钩子文件，SKILL.md 和代理定义 `.md` 的 YAML frontmatter 也支持内联 `hooks` 字段。这允许技能和代理自带钩子配置，无需额外的 `hooks.json` 文件。
...
hooks:
  PostToolUse:
    - matcher: "Read"
      hooks:
        - type: prompt
          prompt: "分析刚读取的文件，关注安全问题和代码质量"
---

你是一个专注于代码质量的审查代理...
```

Frontmatter 中的钩子与文件级钩子合并，优先级低于 `settings.json` 中的同名钩子。
