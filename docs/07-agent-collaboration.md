...
Codara 随附三个内置代理类型，作为 **standalone agent 定义**分发到 `~/.codara/agents/`：

| 类型 | 位置 | 只读 | 默认模型 | 描述 |
|------|------|------|----------|------|
| `Explore` | `~/.codara/agents/Explore.md` | 是 | haiku | 快速代码库探索。文件搜索、代码搜索、理解代码库结构。 |
| `Plan` | `~/.codara/agents/Plan.md` | 是 | 继承主 Agent | 软件架构师。设计实现方案，只读。 |
| `general-purpose` | `~/.codara/agents/general-purpose.md` | 否 | 继承主 Agent | 全能力代理。拥有所有工具，用于复杂多步骤任务。 |

**关键设计**：这些内置代理类型不是硬编码的——它们是普通的 standalone agent 定义文件，遵循标准的代理发现机制。用户可以通过创建同名文件完全覆盖内置行为。

**与 Skills 的区别**：
- **内置代理类型**（Explore/Plan/general-purpose）是通用的代理能力配置，位于 `~/.codara/agents/`
- **技能中的代理**（如 code-review skill 的专用代理）是特定工作流的专用代理，位于 `.codara/skills/*/agents/`

**扩展方式**：
- 新增通用代理类型：在 `~/.codara/agents/` 添加 `MyAgent.md`
- 技能专用代理：在 `.codara/skills/my-skill/agents/` 添加代理定义
...
特殊值 `"inherit"` 解析为主 Agent 的模型。所有解析通过中央路由器完成，详见 [02-模型路由](./02-model-routing.md)。
...
1. **项目 standalone**：`.codara/agents/{subagent_type}.md` → 如果存在，使用自定义定义
2. **项目 skill agents**：`.codara/skills/*/agents/{subagent_type}.md` → 技能中的代理定义
3. **用户 standalone**：`~/.codara/agents/{subagent_type}.md` → 用户级别定义（**内置代理在这里**）
4. **用户 skill agents**：`~/.codara/skills/*/agents/{subagent_type}.md` → 用户技能中的代理定义

内置类型（Explore、Plan、general-purpose）本质上是**默认的 standalone agent 定义**，安装时部署到 `~/.codara/agents/`。它们和自定义代理没有区别——只是预配置了工具集、模型和系统提示词。用户可以通过创建同名的项目/用户 agent 定义来完全覆盖内置行为。

**无硬编码回退**：如果在上述 4 个位置都找不到代理类型，系统会报错，而不是回退到硬编码定义。这确保了所有代理类型都是可见、可覆盖的。

### 内置类型预配置

内置代理类型的默认配置（位于 `~/.codara/agents/*.md`）：
...
自定义代理定义为带有 YAML 前置元数据的 Markdown 文件，从四个位置加载（按优先级顺序）：

1. **项目 standalone：** `.codara/agents/*.md`（最高优先级）
2. **项目 skill agents：** `.codara/skills/*/agents/*.md`
3. **用户 standalone：** `~/.codara/agents/*.md`
4. **用户 skill agents：** `~/.codara/skills/*/agents/*.md`

同名代理按优先级覆盖。技能中的代理定义与 standalone 代理定义格式完全相同。
...
完整的中间件清单详见 [05-中间件与钩子](./05-hooks.md)。
