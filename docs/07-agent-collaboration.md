...
Codara 随附三个内置代理类型，作为**内置 skills** 分发到 `~/.codara/skills/`：

| 类型 | 位置 | 只读 | 默认模型 | 描述 |
|------|------|------|----------|------|
| `Explore` | `~/.codara/skills/explore/agents/Explore.md` | 是 | haiku | 快速代码库探索。文件搜索、代码搜索、理解代码库结构。 |
| `Plan` | `~/.codara/skills/plan/agents/Plan.md` | 是 | 继承主 Agent | 软件架构师。设计实现方案，只读。 |
| `general-purpose` | `~/.codara/skills/general-purpose/agents/general-purpose.md` | 否 | 继承主 Agent | 全能力代理。拥有所有工具，用于复杂多步骤任务。 |

**关键设计**：这些内置代理类型不是硬编码的——它们是普通的 skill agents，遵循标准的代理发现机制。用户可以通过创建同名 skill 完全覆盖内置行为。

**扩展方式**：新增内置代理类型只需在 `~/.codara/skills/` 添加新的 skill 目录和 agent 定义文件，无需修改核心代码。这与 Codara 的核心理念一致：**核心通用，领域扩展全靠 Skill**。
...
3. **用户 standalone**：`~/.codara/agents/{subagent_type}.md` → 用户级别定义
4. **用户 skill agents**：`~/.codara/skills/*/agents/{subagent_type}.md` → 用户技能中的代理定义（**内置 skills 在这里**）

内置类型（Explore、Plan、general-purpose）本质上是**默认的 agent 定义**，作为内置 skills 分发到 `~/.codara/skills/`。它们和自定义代理没有区别——只是预配置了工具集、模型和系统提示词。用户可以通过创建同名的项目/用户 agent 定义来完全覆盖内置行为。

**无硬编码回退**：如果在上述 4 个位置都找不到代理类型，系统会报错，而不是回退到硬编码定义。这确保了所有代理类型都是可见、可覆盖的。

### 内置类型预配置

内置代理类型的默认配置（位于 `~/.codara/skills/*/agents/*.md`）：
