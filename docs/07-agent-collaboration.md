...
| `Plan` | 是 | 继承主 Agent | 软件架构师。设计实现方案，只读。 |
| `general-purpose` | 否 | 继承主 Agent | 全能力代理。拥有所有工具，用于复杂多步骤任务。 |
...
5. **内置默认**：匹配内置预配置（Explore、Plan、general-purpose）

内置类型本质上是**默认的 agent 定义**，和自定义的没有区别——只是预配置了工具集、模型和系统提示词。用户可以通过创建同名的 `.codara/agents/Explore.md` 来覆盖内置 Explore 类型。
