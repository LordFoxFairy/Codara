...
- 用户可以通过创建 `.codara/skills/builtin-agents/` 覆盖整个 skill，或创建 `.codara/agents/Explore.md` 覆盖单个代理

**扩展方式**：
- 新增内置代理类型：在 `builtin-agents/agents/` 添加新的 .md 文件
- 自定义代理类型：在 `.codara/agents/` 或 `.codara/skills/my-skill/agents/` 添加定义
...
4. **用户 skill agents**：`~/.codara/skills/*/agents/{subagent_type}.md` → 用户技能中的代理定义（**builtin-agents skill 在这里**）

内置类型（Explore、Plan、general-purpose）位于 `builtin-agents` skill 中（`~/.codara/skills/builtin-agents/agents/*.md`）。它们和自定义代理没有区别——只是预配置了工具集、模型和系统提示词。用户可以通过创建同名的项目/用户 agent 定义来完全覆盖内置行为。
