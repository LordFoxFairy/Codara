...
- 主代理与从代理在上下文、工具、权限、生命周期上的隔离模型。
- Task/AgentOutput/TodoWrite/Task* 在协作中的分工边界。
- 如何通过 skills 的 agents/hook/permission 编排协作策略。

### 建议阅读方式

1. 先读「主从代理架构」和「关键区分」建立基础心智模型。
2. 再读「Task 工具 + 权限模式 + 工具继承」掌握实际调度行为。
3. 最后看「扩展方式」理解如何覆盖内置代理并引入自定义代理。

### 完成标志

- 你能判断一个任务应在主代理执行还是委派给某类从代理。
- 你能解释从代理为何不能再生成子代理和直接询问用户。

### 最小实操

1. 用 `Task(subagent_type=\"Explore\")` 执行一次只读探索任务，验证工具限制生效。
2. 再用 `general-purpose` 执行一次小改动任务，比较两者工具集差异。
3. 对比前台与后台模式，观察 `AgentOutput` 的结果获取路径。

### 常见误区

- 把可本地完成的小任务过早拆成 subagent，增加上下文和调度成本。
- 忽略从代理工具过滤规则，误判“能力缺失”为系统故障。

### 排错清单（症状 -> 排查顺序）

| 症状 | 排查顺序 |
|------|----------|
| 从代理无法执行预期工具 | 检查工具继承与过滤链 -> 检查代理类型附加 deny 规则 |
| 后台代理结果拿不到 | 检查 `run_in_background` 是否开启 -> 检查 `AgentOutput` 查询 ID |
| 子代理请求权限但主界面无弹窗 | 检查权限事件是否代理到主 Agent -> 检查 TUI 事件消费 |
...
加载顺序：先 `user` 级（`~/.codara/agent-memory/code-reviewer/MEMORY.md`），再 `project` 级（`.codara/agent-memory/code-reviewer/MEMORY.md`）。后加载的内容补充而非覆盖先加载的内容。
