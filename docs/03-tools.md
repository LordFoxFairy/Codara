...
| | TodoWrite | ToolRegistry | 代理自身进度跟踪（详见 [06](./06-agent-collaboration.md)） |
| | TaskCreate, TaskUpdate, TaskList | ToolRegistry | 持久化任务管理（详见 [06](./06-agent-collaboration.md)） |
| | Task, AgentOutput | ToolRegistry | 从代理生命周期（详见 [06](./06-agent-collaboration.md)） |
...
- 写入前由 CheckpointMiddleware 自动创建文件快照（详见 [07-记忆与上下文](./07-memory-system.md)）。
...
每个用户提示创建一个新检查点。该轮次内的文件修改在更改前被快照。回退时恢复目标检查点以来更改的所有文件。完整机制详见 [07-记忆与上下文](./07-memory-system.md)。
