...
| 项 | 契约定义 |
|---|---|
| 输入 | 工具名 + 结构化参数 + 当前运行上下文 |
| 输出 | 可序列化结果字符串/结构（供模型继续推理） |
| 主路径 | 参数校验 -> 权限与钩子链 -> 工具执行 -> 结果回写 |
| 失败路径 | 参数非法、权限拒绝、执行超时、系统错误 |
...
1. 工具不直接做权限决策，授权只在权限层求值。
2. 工具调用前必须经过统一钩子链（至少 Pre/Post ToolUse）。
3. 工具结果必须回写消息上下文，供下一轮推理使用。
4. 写工具必须在变更前完成快照，保证可回退。
5. 工具失败必须返回可解释错误，不允许静默吞错。

## 工具结果最小约束

无论工具内部实现如何，返回结果应满足以下最小约束：

1. 可序列化：可写入会话消息与日志系统。
2. 可判定：成功/失败状态明确。
3. 可解释：错误包含原因与必要上下文。
4. 可截断：超长输出需有稳定截断策略与标记。
5. 可追踪：工具名、调用来源、时间点在事件层可关联。
...
所有工具调用在执行前经过权限检查和钩子拦截。权限检查（deny/ask/allow）、Shell 钩子（PreToolUse/PostToolUse）、文件检查点等拦截逻辑按顺序执行。工具本身只负责"做事"，不关心谁在检查它。完整流程详见 [02-代理循环](./02-agent-loop.md) 的工具执行流程章节。
...
- 写入前由 CheckpointMiddleware 自动创建文件快照（详见 [05-记忆系统](./05-memory-system.md)）。
...
| 参数 | 类型 | 必填 | 说明 |
|------|------|------|------|
| `pattern` | string | 是 | glob 模式（例如 `"**/*.md"`） |
...
每个用户提示创建一个新检查点。该轮次内的文件修改在更改前被快照。回退时恢复目标检查点以来更改的所有文件。完整机制详见 [05-记忆系统](./05-memory-system.md)。
