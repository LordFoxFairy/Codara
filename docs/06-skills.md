...
建议每次技能调用都创建独立的运行域（Skill Runtime Scope），最小包含：

1. `skill_invocation_id`
2. 临时权限集快照
3. 提示展开产物快照
4. 调用期间事件索引
...
1. 调用结束后运行域必须释放，不得污染后续普通对话。
2. 运行域内的临时权限只能影响当前调用窗口。
3. 异常退出时优先执行“权限回收 + 状态落盘”收尾。

产品收益：减少“技能用完后行为异常”的体感。  
开发收益：可按 `skill_invocation_id` 精确追踪问题链路。

## 冲突裁决规则

### 多来源规则冲突

当同一工具调用同时受到多个来源影响时，按以下优先级裁决：

1. 显式 `deny`（用户/项目权限规则）  
2. `ask`（需要人工决策）  
3. 临时 `allow`（skills `allowed-tools`）  
4. 只读豁免与模式兜底规则

同级冲突时：

1. 更具体的模式优先（例如 `Bash(git push*)` 高于 `Bash(git *)`）。
2. 项目级定义优先于用户级定义。
3. 若仍无法区分，采用更保守结果（优先拒绝或询问）。

### 多技能编排冲突

默认推荐“一个技能负责一次完整策略窗口”。若需要组合能力，优先做编排技能，而不是在一次执行中临时叠加多个独立技能定义。
...
| 失败点 | 行为 | 恢复要求 |
|---|---|---|
| 技能不存在 | 终止技能调用并提示可用技能 | 不激活临时权限 |
| 模板展开失败 | 终止本次调用 | 不写入临时状态 |
| 动态命令注入失败 | 用失败占位符替换并继续，或按策略终止 | 必须记录失败来源 |
| 临时权限激活失败 | fail-closed：拒绝进入执行阶段 | 返回明确错误原因 |
| 技能执行中断/异常 | 立即进入收尾 | 必须回收临时 `allowed-tools` |
| 会话异常结束 | 会话销毁流程兜底回收 | 不保留残留临时权限 |
...
即使 Core Events 是固定集，skills hooks 仍是主要扩展手段，因为它负责“**把场景策略绑定到稳定事件点**”，而不是新增事件类型。

职责边界：

1. 订阅 Core Events 或已注册的 Extension Events。
2. 在事件触发时执行项目/团队策略（校验、阻断、改写、审计、通知）。
3. 通过技能目录把脚本、规则、文档打包为可复用能力。
4. 保持核心循环稳定，避免在 Agent Loop 中写业务特判。

非职责：

1. 不直接定义新的生命周期语义。
2. 不绕过权限链和主循环调度。
3. 不修改 Core Events 的语义契约。

如果确实需要新增事件，应先按 [04-生命周期钩子](./04-hooks.md) 的扩展事件注册契约在运行时注册，再由 skills hooks 消费该事件。
...
6. **组合使用**：优先通过一个“编排 skill”组合多个能力，避免临时叠加独立 skill 造成冲突
