...
describe('skill agent definitions', () => {
  it('应解析 builtin agent definitions', async () => {
  ...
  it('应让 agent definition 工具引用匹配 Codara 默认工具名', async () => {
    const explore = resolveSubagentDefinition(await loadSkillsRuntimeData(createBuiltinAgentStore()), 'Explore');
...
`, 'utf8');
  ...
  it('应从 skill agents 目录解析并覆盖 definition', async () => {
