...
allowed-tools: [Read, Write, AskUserQuestion, Agent, Grep, TaskCreate, TaskUpdate, Skill]
---

# Hookify - Create Hooks from Unwanted Behaviors

**FIRST: Load the writing-rules reference** using the Skill tool with `hookify:writing-rules` to understand rule file format and syntax.
...
For each confirmed behavior, create a `.codara/hookify.{rule-name}.local.md` file:
...
**IMPORTANT**: Rule files must be created in the current working directory's `.codara/` folder, NOT the skill directory.

Use the current working directory (where Claude Code was started) as the base path.

1. Check if `.codara/` directory exists in current working directory
   - If not, create it first with: `mkdir -p .codara`

2. Use Write tool to create each `.codara/hookify.{name}.local.md` file
   - Use absolute path: `{cwd}/.codara/hookify.{name}.local.md`
   - The path should resolve to the project's .codara directory, not the skill's

3. Show user what was created:
   ```
   Created 3 hookify rules:
   - .codara/hookify.dangerous-rm.local.md
   - .codara/hookify.console-log.local.md
   - .codara/hookify.sensitive-files.local.md
...
4. Create `.codara/hookify.dangerous-rm.local.md`:
   ```markdown
   ---
...
- **File location**: Create files in project's `.codara/` directory (current working directory), NOT the skill's .codara/
...
2. Ensure `.codara/` directory exists (create with mkdir if needed)
3. Use absolute path: `{cwd}/.codara/hookify.{name}.local.md`
4. Verify file was created with Glob or ls

**If rule doesn't trigger after creation:**
1. Verify file is in project `.codara/` not skill `.codara/`
...
Use TaskCreate and TaskUpdate to track your progress through the steps.
