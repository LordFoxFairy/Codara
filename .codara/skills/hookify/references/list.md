...
1. Use Glob tool to find all hookify rule files:
   ```
   pattern: ".codara/hookify.*.local.md"
   ```
...
**Status**: ✅ Active
**File**: .codara/hookify.dangerous-rm.local.md
```
...
2. Or manually create `.codara/hookify.my-rule.local.md` files
...
Check the skill's `references/examples/` directory for example rule files.
```
