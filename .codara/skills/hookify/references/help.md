...
These hooks read configuration files from `.codara/hookify.*.local.md` and check if any rules match the current operation.

### 2. Configuration Files

Users create rules in `.codara/hookify.{rule-name}.local.md` files:
...
Create `.codara/hookify.my-rule.local.md` with the format above.
...
**Rule Files**: Keep rules in `.codara/hookify.*.local.md` - they should be git-ignored (add to .gitignore if needed).
...
**Hook not triggering:**
- Check rule file is in `.codara/` directory
...
4. Refine the rule by editing `.codara/hookify.warn-rm.local.md`

5. Create more rules as you encounter unwanted behaviors

For more examples, check the skill's `references/examples/` directory.
